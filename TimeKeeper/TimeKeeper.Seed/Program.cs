...
namespace TimeKeeper.Seed
{
    class Program
    {
        ...
        {
            ExcelReader.DataSource = @"C:\Users\muhamedbr\Documents\Dakota Git Repository\Timer.xlsx";
        ...
        private static void SeedCalendar()
        {
            ...
            foreach (DataRow row in rawData.Rows)
            {
                ...
                context.Calendar.Add(day);
                context.SaveChanges();
                if (++N % 100 == 0) Console.Write($"{N} ");
                calendarDictionary.Add(oldId, day.Id);
            }
            Console.WriteLine(context.Calendar.Count());
        }
        ...
        private static void SeedDetails()
        {
            ...
            foreach (DataRow row in rawData.Rows)
            {
                context.Details.Add(new Detail()
                {
                    ...
                    Day = context.Calendar.Find(calendarDictionary[(int)row.Read(3, typeof(int))]),
                });
                if (++N % 100 == 0) Console.Write($"{N} ");
            }
            context.SaveChanges();
            Console.WriteLine(context.Details.Count());
        }
    }
}
